...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen bg-white border-r border-slate-200/60 shadow-sm z-50 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-6 border-b border-slate-200/60">
          <div className="flex items-center gap-3">
            ...
            <span className={`font-bold text-slate-900 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'lg:opacity-0 hidden lg:block'}`}>
              Shelby
            </span>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`relative group flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                  isActive
                    ? 'bg-indigo-50 text-indigo-600 shadow-md shadow-indigo-500/20'
